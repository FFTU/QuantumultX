...
let getCookie = () => {
    ...
    const config = {
        ...
        videoqq: {
            cookie: 'chavy_cookie_videoqq',
            name: '腾讯视频Cookie',
            Host: 'rr.tv'
        },
        fandeng: {
            cookie: 'senku_signbody_pandeng',
            name: '樊登读书Cookie',
            Host: 'api.dushu.io'
        },
        zimuzu: {
            cookie: 'chavy_cookie_zimuzu',
            name: '字幕组Cookie',
            Host: ''
        }
    }
...
let execute = () => {
    ...
    const config = {
        ...
        kafan: {
            ...
            name: '卡饭论坛',
            provider: {
                url: `https://bbs.kafan.cn/plugin.php?id=dsu_amupper&ppersubmit=true&formhash=729db06e&mobile=2`,
                headers: {
                    "cookie": '',
        ...
        fandeng: {
            cookie: 'senku_signbody_pandeng',
            signheaderKey: 'senku_signheader_pandeng',
            name: '樊登读书',
            provider: {
                url: 'https://api.dushu.io/CheckIn',
                method: 'POST',
                headers: "",
                body: ""
            },
        ...
        zimuzu: {
            cookie: 'chavy_cookie_zimuzu',
            cookieAppKey: 'chavy_cookie_zimuzu_app',
            authUrlAppKey: 'chavy_auth_url_zimuzu_app',
            name: '字幕组',
            provider: {
                signweb: {
                    url: 'http://www.rrys2019.com/user/login/getCurUserTopInfo',
                    method: 'GET',
                    headers: {
                        'Cookie': "",
                ...
                signapp: {
                    ...
                    headers: {
                        ...
                        'Accept-Language': "zh-Hans-CN;q=1.0, en-US;q=0.9",
                        'Connection': "close",
                        'Host': "ios.zmzapi.com",
                        'Referer': "http://h5.rrhuodong.com/mobile/mission/pages/task.html",
                        'Host': "ios.zmzapi.com",
                        'User-Agent': 'YYets_swift/2.5.7 (com.yyets.ZiMuZu; build:29; iOS 13.3.1) Alamofire/4.9.1'
                    }
                },
...
[${res.bar}] ${res.errorCode == 0 ? '签到成功' : '签到失败'}，${res.errorCode == 0 ? res.errorMsg : ('原因：' + res.errorMsg)}`;
    ...
    let sign_kafan = () => {
        if (!global.sign.kafan) {
            record(`[${config.kafan.name}]未开启签到`);
            return;
        }
        let cookieVal = $prefs.valueForKey(config.kafan.cookie);
        //console.log('CookieKAFAN=>' + cookieVal);
        if (!cookieVal) {
            config.kafan.data.notify = `[${config.kafan.name}] 未获取到Cookie⚠️`;
            record(config.kafan.data.notify);
            finalNotify("kafan");
            return;
        }
        config.kafan.provider.headers.cookie = cookieVal;
        $task.fetch(config.kafan.provider).then(response => {
            try {
                //console.log('KAFAN=>' + response.body);
                if (response.body.match(/\u7D2F\u8BA1\u7B7E\u5230/)) {
                    //success
                    config.kafan.data.notify = `[${config.kafan.name}] 签到成功🎉`;
                } else if (response.body.match(/\u65E0\u9700\u518D\u6B21\u7B7E\u5230/)) {
                    //repeat
                    config.kafan.data.notify = `[${config.kafan.name}] 重复签到🎉`;
                } else if (response.body.match(/\u62D2\u7EDD/)) {
                    //cookie
                    config.kafan.data.notify = `[${config.kafan.name}] 请求错误⚠️`;
                } else {
                    //script need update
                    config.kafan.data.notify = `[${config.kafan.name}] 脚本需更新⚠️`;
                }
                record(config.kafan.data.notify);
            } catch (e) {
                config.kafan.data.notify = `[${config.kafan.name}] 脚本数据解析异常⚠️`;
                record(`${config.kafan.data.notify}-${JSON.stringify(e)}`)
            }
            finalNotify("kafan");
        }, reason => {
            config.kafan.data.notify = `[${config.kafan.name}] 签到失败！网络请求异常⚠️`;
            finalNotify("kafan");
            record(`${config.kafan.data.notify} : ${reason.error}`);
        });
    }
    ...
    //#region 樊登读书
    let sign_fandeng = () => {
        if (!global.sign.fandeng) {
            record(`[${config.fandeng.name}] 未开启签到`);
            return;
        }
        let signheaderVal = $prefs.valueForKey(config.fandeng.signheaderKey);
        let signBodyVal = $prefs.valueForKey(config.fandeng.cookie);

        if (!signBodyVal) {
            config.fandeng.data.notify = `[${config.fandeng.name}] 未获取到Cookie⚠️`;
            record(config.fandeng.data.notify);
            finalNotify("fandeng");
            return;
        }
        ...
        let checkSign = () => {
            ...
            config.fandeng.provider.headers = JSON.parse(signheaderVal);
            config.fandeng.provider.body = signBodyVal;

            $task.fetch(config.fandeng.provider).then(response => {
                try {
                    let data = JSON.parse(response.body);
                    if (data.status == 1) {
                        config.fandeng.data.notify = `[${config.fandeng.name}] 签到成功🎉`
                        record(config.fandeng.data.notify);
                        finalNotify("fandeng");
                    } else {
                        config.fandeng.data.notify = `[${config.fandeng.name}] 签到状态：${data.message}`
                        record(config.fandeng.data.notify);
                        finalNotify("fandeng");
                    }
                } catch (e) {
                    config.fandeng.data.notify = `[${config.fandeng.name}] 签到失败！数据处理异常⚠️`;
                    record(`${config.fandeng.data.notify}-${JSON.stringify(e)}`);
                    finalNotify("fandeng");
                }
            }, reason => {
                config.fandeng.data.notify = `[${config.fandeng.name}] 签到失败！网络请求异常⚠️`;
                record(`${config.fandeng.data.notify}-${reason.error}`);
                finalNotify("fandeng");
            })
        }
        checkSign();
    ...
    //#region 字幕组
    let sign_zimuzu = () => {
        if (!global.sign.zimuzu) {
            record(`[${config.zimuzu.name}] 未开启签到`);
            return;
        }
        let cookieAppVal = $prefs.valueForKey(config.zimuzu.cookieAppVal);
        let authUrlAppVal = $prefs.valueForKey(config.zimuzu.authUrlAppKey);
        let cookieVal = $prefs.valueForKey(config.zimuzu.cookie);
        let zimuzu_signinfo = {}
        if (!cookieVal) {
            config.zimuzu.data.notify = `[${config.zimuzu.name}] 未获取到Cookie⚠️`;
            record(config.zimuzu.data.notify);
            finalNotify("zimuzu");
            return;
        }
        function signweb () {
            ...
            chavy.get(url, (error, response, data) => {
                //chavy.log(`${config.zimuzu.name}, signweb - data: ${data}`)
                zimuzu_signinfo.web = JSON.parse(data)
            })
        }
        function loginapp (cb) {
            if (authUrlAppVal) {
                ...
                chavy.get(url, (error, response, data) => {
                    ...
                    else zimuzu_signinfo.app = result
                })
            } else {
                cb()
            }
        ...
        function refreshapp (logininfo, cb) {
            ...
            chavy.get(url, (error, response, data) => {
                ...
                if (respcookie && respcookie.indexOf('PHPSESSID=') >= 0) {
                    ...
                    if (cookieAppVal) chavy.setdata(cookieAppVal, config.zimuzu.cookieAppKey)
                }
                cb()
            })
        }
        function signapp () {
            loginapp(() => {
                ...
                chavy.get(url, (error, response, data) => {
                    zimuzu_signinfo.app = JSON.parse(data)
                })
            })
        }
        function getinfo () {
            const title = `${config.zimuzu.name}`
            let subTitle = `网页: `
            let detail = ''

            // web
            if (zimuzu_signinfo.web.status == 1) {
                if (zimuzu_signinfo.web.data.new_login) subTitle += '成功'
                else subTitle += '成功 (重复)'
                detail = `人人钻: ${zimuzu_signinfo.web.data.userinfo.point}, 登录天数: ${zimuzu_signinfo.web.data.usercount.cont_login}`
            } else if (zimuzu_signinfo.web.status == 4001) {
                subTitle += '未登录'
            } else {
                subTitle += '失败'
            }
            ...
            // app
            subTitle += `; APP: `
            if (zimuzu_signinfo.app.status == 1) subTitle += '成功'
            else if (zimuzu_signinfo.app.status == 4005) subTitle += '成功 (重复)'
            else if (zimuzu_signinfo.app.status == 1021) subTitle += '未登录'
            else subTitle += '失败'
            config.zimuzu.data.notify = `[${config.zimuzu.name}] ${subTitle} ${detail}`;
            record(`${config.zimuzu.data.notify}`);
            finalNotify("zimuzu");
        ...
        function check (checkms = 0) {
            if (zimuzu_signinfo.web && zimuzu_signinfo.app) {
                getinfo()
            } else {
                if (checkms > 5000) {
                    config.zimuzu.data.notify = `[${config.zimuzu.name}] 签到失败: 超时退出`;
                    record(`${config.zimuzu.data.notify}`);
                    finalNotify("zimuzu");
    ...
    let startSign = () => {
        if (global.parallel) {
            ...
            if (global.sign.videoqq) sign_videoqq();
            if (global.sign.fandeng) sign_fandeng();
            if (global.sign.zimuzu) sign_zimuzu();
            if (global.sign.kafan) sign_kafan();
        } else {
            if (global.sign.baidu_tieba) sign_baidu_tieba();
            ...
            else if (global.sign.videoqq) sign_videoqq();
            else if (global.sign.fandeng) sign_fandeng();
            else if (global.sign.zimuzu) sign_zimuzu();
            else if (global.sign.kafan) sign_kafan();
