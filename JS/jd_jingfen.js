...
感谢@yichahucha 的jd_price.js
本脚本是在jd_price.js 的脚本基础上增加了京粉返佣转链的功能，可以给自己省钱

获取京粉Cookie 
https://raw.githubusercontent.com/FFTU/QuantumultX/master/Get_Cookie.conf
[mitm]
hostname = qwd.jd.com

[rewrite_local]
#京东京粉
^https?://api\.m\.jd\.com/client\.action\?functionId=(wareBusiness|serverConfig) url script-response-body FFTU/JS/jd_jingfen.js
...
const $tool = tool();
const cookieKey = 'chavy_cookie_jingfen'
const cookieVal = $tool.read(cookieKey)
...
if (url.indexOf(path2) != -1) {
    ...
    request_history_price(shareUrl, function (data) {
        ...
        request_jingfen_price(shareUrl, function (data) {
              if (data.errCode==0) {
                   const content = data.content;
                   const commission = data.skuInfos[0].commission.toFixed(2).split(" ");
                   $tool.notify("", "", `${content}`+"————————\n预计佣金：￥"+`${commission}` );
              }else{
                   console.log("JingFen ERROR:\n" + data.msg);
              }    
...
function request_jingfen_price(share_url, callback) {    
    const options = {
        url: "http://qwd.jd.com/cps/zl?content="+encodeURIComponent(share_url)+"shareSource=1_2_2",
        headers: {
            "Cookie": cookieVal
        },
